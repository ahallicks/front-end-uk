...
export const AuthorCard: React.FC<IAuthorCard> = ({
	...
	<section className="segment mx-auto max-w-7xl px-6 lg:px-8">
		<div className="flex items-center justify-center dark:bg-gray-900">
			<div className="relative flex w-full max-w-2xl flex-col items-start space-y-4 rounded-lg border-2 border-dashed border-gray-400 px-4 py-8 shadow-lg sm:flex-row sm:space-y-0 sm:space-x-6 dark:border-gray-400">
				<span className="bg-primary-100 absolute top-0 left-0 rounded-tl-lg rounded-br-lg border-r-2 border-b-2 border-dashed border-gray-400 px-2 py-1 text-xs font-medium dark:border-gray-400 dark:bg-gray-900 dark:text-gray-300">
					author
				</span>
				...
				<div className="flex w-full shrink-0 justify-center sm:w-auto sm:justify-start">
					{author.image ? (
						<img
							className="mt-3 mr-3 h-20 w-20 rounded-full object-cover"
							alt=""
							src={author.image.url}
				...
				<div className="flex w-full flex-col items-center sm:w-auto sm:items-start">
					...
					<div
						className="prose mb-4 text-center text-gray-400 sm:text-left md:text-lg dark:prose-invert"
						data-hygraph-entry-id={contentId}
						data-hygraph-field-api-id="author"
						data-hygraph-component-chain={`[{"fieldApiId":"Sections","instanceId":"${id}"}]`}
					>
