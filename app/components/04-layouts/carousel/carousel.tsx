...
export const Carousel: React.FC<TCarouselProps> = ({
	...
	return (
		<Tag
			...
			className={`carousel mx-auto scrollbar-thin grid max-w-2xl scrollbar-thumb-slate-300 scrollbar-track-transparent grid-flow-col gap-4 overflow-x-auto overscroll-x-contain scroll-smooth pt-4 pb-10 scrollbar-thumb-rounded-full scrollbar-track-rounded-full lg:mx-0 lg:max-w-none xl:gap-8 dark:border-gray-700 ${hasScrollbar ? 'cursor-grab' : ''} ${isDragging ? 'isDragging' : ''} ${axis === 'vertical' ? 'vertical' : ''} ${className}`}
		>
			{children}
		</Tag>
	);
